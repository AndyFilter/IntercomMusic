...
namespace ImGui
{
    ...
    IMGUI_API bool          Selectable(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0), bool border = false); // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
    IMGUI_API bool          Selectable(const char* label, bool* p_selected, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0), bool border = false);      // "bool* p_selected" point to the selection state (read-write), as a convenient helper.
...
enum ImGuiSelectableFlags_
{
    ...
    ImGuiSelectableFlags_UseSpacing         = 1 << 5,   // Don't pack them so closely
};
